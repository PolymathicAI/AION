...
from .text_utils import get_sentinel_to_id_mapping, merge_span_masking
...
def init_empty_target_modality(
    mod_dict, modality_info, domain, batch_size, num_tokens, device
):
    """
    ...
    if modality_info[domain]["type"] == "img":
        ...
        mod_dict[domain] = {
            "tensor": torch.zeros(
                (batch_size, num_tokens), dtype=torch.int32, device=device
            ),
            "input_mask": torch.ones(
                (batch_size, num_tokens), dtype=torch.bool, device=device
            ),
...
def init_full_input_modality(mod_dict, modality_info, domain, device, eos_id=3):
    if "input_mask" not in mod_dict[domain]:
        mod_dict[domain]["input_mask"] = torch.zeros(
            mod_dict[domain]["tensor"].shape, dtype=torch.bool, device=device
        )
    if "target_mask" not in mod_dict[domain]:
        mod_dict[domain]["target_mask"] = torch.ones(
            mod_dict[domain]["tensor"].shape, dtype=torch.bool, device=device
        )
    if "decoder_attention_mask" not in mod_dict[domain]:
        mod_dict[domain]["decoder_attention_mask"] = torch.zeros(
            mod_dict[domain]["tensor"].shape, dtype=torch.bool, device=device
        )
    ...
    if modality_info[domain]["type"] == "img":
        ...
        mod_dict[domain]["target_mask"] = torch.ones_like(
            mod_dict[domain]["mask_valid"]
        )
        mod_dict[domain]["decoder_attention_mask"] = torch.zeros_like(
            mod_dict[domain]["mask_valid"]
        )
...
def custom_text(
    ...
    sample[key]["decoder_attention_mask"] = torch.zeros(
...
class GenerationSampler(nn.Module):
    ...
    def select_tokens_batched(
        ...
        if logits.ndim > 2:
            ...
            return self.sample_tokens(
                logits, num_select, temperature, top_k, top_p, return_all_samples
            )
    ...
    def forward_mask_encoder_generation(self, encoder_mod_dict):
        ...
        if self.model.num_register_tokens > 0:
            ...
            encoder_emb = torch.cat(
                [torch.zeros_like(prompt_tokens), encoder_emb], dim=1
            )
            encoder_mask = torch.cat(
                [
                    torch.zeros(
                        (B, prompt_tokens.shape[1]),
                        dtype=torch.bool,
    ...
    def forward_mask_decoder_maskgit(self, mod_dict, target_mod, seed=None):
        ...
        mod_mask_all = torch.full_like(
            d["ids"], self.model.modality_info[target_mod]["id"], dtype=torch.int16
        )
    ...
    def maskgit_step_batched(
        ...
        top_samples, top_indices = self.select_tokens_batched(
            logits, num_select, temperature=temperature, top_k=top_k, top_p=top_p
        )
        # Update mod dict
    ...
    def multi_guided_roar_step_batched(
        ...
        select_pos = mod_pos
        uncond_dict[target_mod]["tensor"] = torch.scatter(
            uncond_dict[target_mod]["tensor"], -1, select_pos, samples
        )
        uncond_dict[target_mod]["input_mask"] = torch.scatter(
            uncond_dict[target_mod]["input_mask"],
            -1,
        ...
        for i in range(len(cond_dicts)):
            cond_dicts[i][target_mod]["tensor"] = torch.scatter(
                cond_dicts[i][target_mod]["tensor"], -1, select_pos, samples
            )
            cond_dicts[i][target_mod]["input_mask"] = torch.scatter(
                cond_dicts[i][target_mod]["input_mask"],
                -1,
    ...
    def autoregressive_step_batched(
        ...
        if use_eos and eos_token is None:
            ...
            eos_token = decoder_ids[0][decoder_mask[0] == 0][
                -1
            ]  # Assumes the EOS token is the same for all
        if use_eos:
    ...
    def guided_autoregressive_step_batched(
        ...
        encoder_tokens, encoder_emb, encoder_mask_cond, encoder_mod_mask = (
            self.forward_mask_encoder_generation(encoder_mod_dict)
        )
        x = encoder_tokens + encoder_emb
        ...
        encoder_tokens, encoder_emb, encoder_mask_uncond, encoder_mod_mask = (
            self.forward_mask_encoder_generation(encoder_mod_dict)
        )
        x = encoder_tokens + encoder_emb
